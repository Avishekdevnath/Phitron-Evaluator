(function(){if(window.__phitronContentLoaded)return;window.__phitronContentLoaded=!0;const E=".ReactModal__Content",I='input[name="obtainMark"]',L=".ql-editor",k="phitron-fill-panel-root",S="phitron-capture-btn";function M(t){if(!t||t.length<2||t.length>100)return!1;const e=t.toLowerCase();return["student","name","assignment","email","phone","date","mark","score","submission","deadline"].some(n=>e.includes(n))?!1:/[a-z]/i.test(t)}function q(t){const e=[],o=t.querySelectorAll('span.badge, [class*="badge"], [class*="chip"]');for(const n of o){const a=(n.textContent?.trim()||"").match(/Q(\d+)\s+(\d+)\/(\d+)/);a&&(e.push({number:a[1],maxMarks:parseInt(a[3],10)}),console.log("[Phitron] Found question from modal:",a[1],a[3]))}return console.log("[Phitron] Extracted",e.length,"questions from modal"),e}function N(t){const e={studentName:"",assignmentName:"",email:"",submissionDate:"",colabLink:"",totalMarks:0,questionsFromModal:[]};console.log("[Phitron] Starting extraction from form:",t);let o=t.querySelector("header");if(o||(o=t.querySelector('[class*="modal-header"]')),o||(o=t.querySelector('[class*="Header"]')),o){const s=o.querySelector("strong");if(s&&(e.assignmentName=s.textContent?.trim()||"",console.log("[Phitron] Found assignment name via header strong:",e.assignmentName)),!e.assignmentName){const u=(o.textContent?.trim()||"").split(`
`);u[0]&&(e.assignmentName=u[0].trim(),console.log("[Phitron] Found assignment name via header text:",e.assignmentName))}}const n=t.querySelectorAll("label");console.log("[Phitron] Found",n.length,"labels in form");for(let s=0;s<n.length;s++){const l=n[s],b=(l.textContent?.trim()||"").toLowerCase();if(b.includes("student name")&&!e.studentName){if(s+1<n.length){const h=n[s+1].textContent?.trim()||"";if(h&&!h.toLowerCase().includes("student name")&&!h.toLowerCase().includes("phone")&&h.length>2){e.studentName=h.split(`
`)[0],console.log("[Phitron] Found student name from next label:",e.studentName);continue}}const g=l.parentElement;if(g&&g.nextElementSibling){const h=g.nextElementSibling.querySelector("label");if(h){const x=h.textContent?.trim()||"";if(x&&!x.toLowerCase().includes("student name")&&x.length>2){e.studentName=x.split(`
...
`)[0],console.log("[Phitron] Found student name via sibling:",e.studentName);continue}}}if(b.includes("email")&&!b.includes("phone")&&!e.email){if(s+1<n.length){const m=n[s+1].textContent?.trim()||"";if(m&&m.includes("@")){e.email=m,console.log("[Phitron] Found email from next label:",e.email);continue}}let g=l.nextElementSibling;if(g){const c=g.textContent?.trim()||"";c&&!c.toLowerCase().includes("email")&&(e.email=c,console.log("[Phitron] Found email:",e.email))}}if(b.includes("submission date")&&!e.submissionDate){if(s+1<n.length){const m=n[s+1].textContent?.trim()||"";if(m&&!m.toLowerCase().includes("submission date")){e.submissionDate=m,console.log("[Phitron] Found submission date from next label:",e.submissionDate);continue}}let g=l.nextElementSibling;if(g){const c=g.textContent?.trim()||"";c&&!c.toLowerCase().includes("submission date")&&(e.submissionDate=c,console.log("[Phitron] Found submission date:",e.submissionDate))}}}if(!e.studentName&&n.length>0){console.log("[Phitron] Using aggressive fallback for student name...");for(let s=0;s<n.length;s++){const u=n[s].textContent?.trim()||"";if(M(u)){e.studentName=u,console.log("[Phitron] Found student name via validation fallback:",u);break}}}const i=t.querySelectorAll("a");for(const s of Array.from(i)){const l=s.getAttribute("href")||"";if(l.includes("colab")){e.colabLink=l,console.log("[Phitron] Found Colab link:",l);break}}const a=t.textContent||"",p=[/Approximate\s+Marks\s*:\s*\d+\s*\/\s*(\d+)/i,/Score\s*:\s*\d+\s*\/\s*(\d+)/i,/out\s+of\s+(\d+)/i];for(const s of p){const l=a.match(s);if(l){e.totalMarks=parseInt(l[1],10),console.log("[Phitron] Found total marks:",e.totalMarks,"via pattern:",s.source);break}}if(!e.totalMarks){const s=t.querySelector(".font-weight-bold.pl-2");if(s){const l=parseInt(s.textContent?.trim()||"",10);!isNaN(l)&&l>0&&(e.totalMarks=l,console.log("[Phitron] Found total marks via .font-weight-bold.pl-2:",l))}}return e.questionsFromModal=q(t),console.log("[Phitron] Extracted submission info:",e),e}function A(t){const e=document.createElement("button");return e.id=S,e.className="btn btn-info btn-sm w-100",e.textContent="📋 Capture Submission Info",e.type="button",e.style.cssText=`
    ...
    `;const l=n.querySelector("#phitron-refresh-btn");return l&&l.addEventListener("click",()=>{console.log("[Phitron] Refresh clicked, remounting panel"),C(o)}),n}function _(t,e,o){const n=document.createElement("div");n.className="phitron-ai-panel-root card mb-3",n.style.borderTop="3px solid #0d6efd",n.style.borderRadius="6px";const i=N(o),a={assignmentName:i.assignmentName||"",email:i.email||""},p=A(o);if(n.appendChild(p),t&&!D(t,a))return n.appendChild(O(t,a,o)),n;if(t){const s=Math.round(t.totalScore/t.maxScore*e),l=e/t.maxScore,u=t.questionResults.map(r=>Math.round(r.maxMarks*l)),b=e-u.reduce((r,d)=>r+d,0);b!==0&&u.length>0&&(u[u.length-1]+=b);const g=t.questionResults.map((r,d)=>{const f=u[d];return r.maxMarks===0?0:Math.min(f,Math.round(r.awardedMarks*(f/r.maxMarks)))}),c=s-g.reduce((r,d)=>r+d,0);if(c!==0){const r=g.findIndex((d,f)=>d+c>=0&&d+c<=u[f]);r>=0&&(g[r]+=c)}const m=t.questionResults.map((r,d)=>`<span class="badge ${r.status==="complete"?"badge-success":r.status==="partial"?"badge-warning":"badge-danger"} mr-1 mb-1">Q${r.questionNumber} ${g[d]}/${u[d]}</span>`).join(""),h=document.createElement("div"),x=t.submissionInfo?.studentName?` for ${t.submissionInfo.studentName}`:"";h.innerHTML=`
      ...
      `,n.appendChild(h);const y=n.querySelector("#phitron-apply-btn");y&&y.addEventListener("click",()=>{T(o,s);const r=[];r.push("<p><strong>Examiner Feedback:</strong> Overall performance evaluated.</p>"),r.push("<p></p>"),t.questionResults.forEach((d,f)=>{r.push(`<p><strong># Question ${d.questionNumber}</strong></p>`),r.push(`<p><em>${d.summary||d.questionNumber}</em> → <strong>${g[f]} / ${u[f]}</strong></p>`),d.mistakes?.[0]&&r.push(`<p><strong>Note:</strong> ${d.mistakes[0]}</p>`),r.push("<p></p>")}),r.push("<p><strong>Important Instructions:</strong></p>"),r.push("<p>→ Do not post on Facebook, if you have any marks-related issues.</p>"),r.push("<p>→ Make sure to read all the requirements carefully, If you have any marks-related confusion.</p>"),r.push("<p>→ If you are confident and If there is a mistake from the examiner's end, give a recheck request.</p>"),r.push("<p>→ If your recheck reason was not valid, 2 marks will be deducted from your current marks.</p>"),r.push("<p>→ Please check the documentation below for more information about how to recheck.</p>"),r.push("<p><br></p>"),r.push('<p style="color:red;"><strong>We have a recheck option, so please refrain from posting to the group.</strong></p>'),r.push('<p style="color:green;"><em>If your recheck reason is valid you will get marks, if not valid 2 marks will be deducted.</em></p>'),F(o,r.join("")),y.textContent="✓ Applied — Click to Re-apply",y.classList.add("disabled"),setTimeout(()=>y.classList.remove("disabled"),1e3)})}else{const s=document.createElement("div");s.innerHTML=`
      ...
      `,n.appendChild(s)}return n}function C(t){const e=document.getElementById(k);e&&e.remove(),console.log("[Phitron] mountPanel called, form element:",t);const o=R(t);$().then(n=>{document.querySelectorAll(`#${k}, .phitron-ai-panel-root`).forEach(l=>l.remove());const i=_(n,o,t);i.id=k,console.log("[Phitron] Created panelUI:",i);let a=!1;const p=t.querySelector(".modal-body");p&&(p.firstChild?p.insertBefore(i,p.firstChild):p.appendChild(i),a=!0,console.log("[Phitron] Panel inserted into .modal-body",p)),!a&&t.firstChild&&(t.insertBefore(i,t.firstChild),a=!0,console.log("[Phitron] Panel inserted into form element")),a||(t.appendChild(i),a=!0,console.log("[Phitron] Panel appended to form element")),console.log("[Phitron] Panel mounted, inserted:",a,"result:",n);const s=document.getElementById(S);console.log("[Phitron] Capture button in DOM:",!!s)})}if(P())try{chrome.storage.onChanged.addListener(t=>{if(!("lastEvaluationResult"in t))return;const e=document.querySelector(E);e&&(console.log("[Phitron] Eval changed, remounting panel"),setTimeout(()=>C(e),100))})}catch(t){console.warn("[Phitron] storage.onChanged listener failed:",t)}new MutationObserver(()=>{const t=document.querySelector(E);t&&!t.getAttribute("data-fillpanel-mounted")&&(t.setAttribute("data-fillpanel-mounted","true"),console.log("[Phitron] Form detected, mounting panel..."),C(t))}).observe(document.body,{childList:!0,subtree:!0});const w=document.querySelector(E);w&&!w.getAttribute("data-fillpanel-mounted")&&(w.setAttribute("data-fillpanel-mounted","true"),console.log("[Phitron] Form already loaded, mounting panel immediately..."),C(w)),console.log("[Phitron Content] Script loaded and monitoring for assignment modals"),console.log("[Phitron] phitronContent loaded - panel will appear when modal opens")})();
